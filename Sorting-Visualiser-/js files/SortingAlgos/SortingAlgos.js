...
function mergeSort(array) {
  ...
  function merge(left, right) {
    ...
    async function mergeSortVisual(array) {
        ...
        const sortedLeft = await mergeSortVisual(left);
        const sortedRight = await mergeSortVisual(right);
    
        return mergeVisual(sortedLeft, sortedRight);
    }
    ...
    function highlightBars(leftIndex, rightIndex) {
        let leftBar = document.getElementsByClassName(`bar${leftIndex}`)[0];
        let rightBar = document.getElementsByClassName(`bar${rightIndex}`)[0];
        leftBar.style.background = "#FFC300";
        rightBar.style.background = "#FFC300";
    }
    
    function updateArrayVisual(array) {
        for (let i = 0; i < array.length; i++) {
            let bar = document.getElementsByClassName(`bar${i}`)[0];
            bar.style.height = `${array[i] * 10}px`;
            bar.style.background = "linear-gradient(to bottom, #36454F, #6082b6cf)";
        }
    }    